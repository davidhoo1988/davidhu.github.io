...
更具体地，假定BGV最开始的噪声幅值大约为x, 即 <img src="https://latex.codecogs.com/svg.image?Err(\mathbf{c_i})=Err(BGV_\mathbf{s}(m_i))\approx&space;x" title="https://latex.codecogs.com/svg.image?Err(\mathbf{c_i})=Err(BGV_\mathbf{s}(m_i))\approx x" /> 。
