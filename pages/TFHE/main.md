...
<div>这里再分析privKS的噪声增长规律。 已知 <img src="https://latex.codecogs.com/svg.image?a_{i,j}^{(k)}\sim&space;Unif(-\frac{B_{ks}}{2},\frac{B_{ks}}{2})" title="https://latex.codecogs.com/svg.image?a_{i,j}^{(k)}\sim Unif(-\frac{B_{ks}}{2},\frac{B_{ks}}{2})" /> 且 <img src="https://latex.codecogs.com/svg.image?\mathbf{KSK}_{k,i,j}^{(f)}\sim&space;\mathcal{N}(0,&space;\sigma_{ksk}^2)" title="https://latex.codecogs.com/svg.image?\mathbf{KSK}_{k,i,j}^{(f)}\sim \mathcal{N}(0, \sigma_{ksk}^2)" />, 容易推出 <img src="https://latex.codecogs.com/svg.image?&space;a_{i,j}^{(k)}\cdot&space;\mathbf{KSK}_{k,i,j}^{(f)}\sim&space;\mathcal{N}(0,&space;\frac{B_{ks}^2\sigma_{ksk}^2}{12})" title="https://latex.codecogs.com/svg.image? a_{i,j}^{(k)}\cdot \mathbf{KSK}_{k,i,j}^{(f)}\sim \mathcal{N}(0, \frac{B_{ks}^2\sigma_{ksk}^2}{12})" />, 因此最终我们有 <img src="https://latex.codecogs.com/svg.image?&space;-\sum_k\sum_i\sum_ja_{i,j}^{(k)}\cdot&space;\mathbf{KSK}_{k,i,j}^{(f)}\sim&space;\mathcal{N}(0,&space;p(n&plus;1)d_{ks}\frac{B_{ks}^2}{12}\sigma_{ksk}^2)" title="https://latex.codecogs.com/svg.image? -\sum_k\sum_i\sum_ja_{i,j}^{(k)}\cdot \mathbf{KSK}_{k,i,j}^{(f)}\sim \mathcal{N}(0, p(n+1)d_{ks}\frac{B_{ks}^2}{12}\sigma_{ksk}^2)" />。这里记作 <img src="https://latex.codecogs.com/svg.image?\sigma_{\mathbf{PrivKS}}^2=&space;p(n&plus;1)d_{ks}\frac{B_{ks}^2}{12}\sigma_{ksk}^2" title="https://latex.codecogs.com/svg.image?\sigma_{\mathbf{PrivKS}}^2= p(n+1)d_{ks}\frac{B_{ks}^2}{12}\sigma_{ksk}^2" />。</div>
...
<div>最后分析Circuit Bootstrap的噪声规律。
