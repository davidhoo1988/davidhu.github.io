...
3. 做Relinearization操作得 <img src="https://latex.codecogs.com/svg.image?\mathbf{ct_\times}\gets&space;Relinarize(Rescale(\mathbf{ct_{\times}}),&space;\{\mathbf{rlk_i}\})" title="https://latex.codecogs.com/svg.image?\mathbf{ct_\times}\gets Relinarize(Rescale(\mathbf{ct_{\times}}), \{\mathbf{rlk_i}\})" /> 。
